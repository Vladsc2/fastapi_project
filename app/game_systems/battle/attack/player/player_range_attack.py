...
async def get_available_targets(
    ...
    if (stopper_checker.check_stopper(room_model, None)
            or stopper_checker.check_stopper(room_model, const.Stopper.Battle.AWAIT_TARGET_RANGE_ATTACK)):
    ...
    return (f"Не удалось обработать событие, во время работы с атакой дальнего боя\n\n"
            f"  {__file__}\n\n"
...
async def range_attack(
    ...
    if stopper_checker.check_stopper(room_model, const.Stopper.Battle.AWAIT_TARGET_RANGE_ATTACK):
        ...
        const.Stopper.Battle.AWAIT_HIT_ROLL_RANGE_ATTACK,
        ...
        const.Stopper.Battle.AWAIT_DAMAGE_ROLL_RANGE_ATTACK,
        const.Stopper.ENEMY_POINTERS,
    ]):
    ...
    return (f"Не удалось обработать событие, во время выбора цели для атаки дальнего боя\n\n"
            f"  {__file__}\n\n"
            f"  - state: {room_model.state}\n"
            f"  - stoppers: {room_model.stoppers}\n")
...
async def _to_select_enemy(
    ...
    if weapon is None:
        return player_attack_text.get_no_range_weapon_text()
    ...
    for index, mob_model in enumerate(enemies):
        text += f" {index+1}) "
        text += f"<a href='/room/range_attack-{index+1}'>{mob_model.name}</a>"
        text += f" (lvl={mob_model.lvl}, hp={mob_model.hp}, mana={mob_model.mana})\n\n"

    text += f"<a href='/room'>Назад</a>"

    room_model.stoppers = [const.Stopper.Battle.AWAIT_TARGET_RANGE_ATTACK]
...
async def _select_target(
    ...
    text = player_attack_text.request_hitting_target_roll(enemy, 'range_attack')
...
async def _hitting_roll_awaiting(
    ...
    if roll_value is None:
        return player_attack_text.request_hitting_target_roll(enemy, 'range_attack')
    ...
    text += " <a href='/room/range_attack-1'>Далее</a>"
